...
class YJAttentionCell: UITableViewCell {
    ...
    lazy var collectionView : UICollectionView = {
        ...
        collectionView.register(YJAttentionPicCell.self, forCellWithReuseIdentifier:YJConst.attendPicCellSI)
    ...
    var eventInfosModel:YJEventInfosModel? {
        didSet {
            guard let model = eventInfosModel else {return}
            self.picView.sd_setImage(with: URL(string: (model.authorInfo?.avatarUrl!)!), completed: nil)
    ...
    func height(for commentModel: YJAContentInfo?) -> CGFloat {
...
extension YJAttentionCell:UICollectionViewDelegate, UICollectionViewDataSource,UICollectionViewDelegateFlowLayout{
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return self.eventInfos?.contentInfo?.picInfos?.count ?? 0
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell:YJAttentionPicCell = collectionView.dequeueReusableCell(withReuseIdentifier: YJConst.attendPicCellSI, for: indexPath) as! YJAttentionPicCell
