...
import UIKit
class YJRecommendCell: UITableViewCell {

    private var findRecommendSteam:YJRStreamList?
    ...
    lazy var collectionView : UICollectionView = {
        ...
        collectionView.register(YJRecommendPicCell.self, forCellWithReuseIdentifier:YJConst.recommendPicCellSI)
    ...
    func setUpLayout(){
        ...
        self.collectionView.snp.makeConstraints { (make) in
            ...
            make.height.equalTo((YJConst.screenWidth-30)/3+20)
        }
    }
    ...
    var streamModel:YJRStreamList? {
        didSet {
            guard let model = streamModel else {return}
            self.picView.sd_setImage(with: URL(string: model.avatar!))
            ...
            let OnePicHeight = CGFloat((YJConst.screenWidth - 30) / 3)
    ...
    // 计算文字的高度
    func height(for commentModel: YJRStreamList?) -> CGFloat {
        ...
        height += label.sizeThatFits(CGSize(width: YJConst.screenWidth - 30, height: CGFloat.infinity)).height
        return height
    }
...
extension YJRecommendCell: UICollectionViewDelegate, UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return self.findRecommendSteam?.picUrls?.count ?? 0
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell:YJRecommendPicCell = collectionView.dequeueReusableCell(withReuseIdentifier: YJConst.recommendPicCellSI, for: indexPath) as! YJRecommendPicCell
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        return CGSize.init(width: (YJConst.screenWidth - 30) / 3, height:(YJConst.screenWidth - 30) / 3)
    }
